...
/**
 * Synthetic weight assigned to bridging relations. Real neighbor bonds carry
 * weights in `[WEIGHT_LOW, 1]` (≈ 0.2 … 1), so a bridge at 0.1 always ranks
 * below them — bridging only joins disconnected components and should be the
 * last thing the layout "pulls together".
 */
const BRIDGE_WEIGHT = 0.1;
 ...
 *   components, plus a low synthetic weight for each; both arrays are empty
 *   when the graph is already connected.
 */
export function bridgeSets(
  neighborOutput: NeighborOutput,
  objects: ReadonlyArray<Rect>,
): { horizontal: Relation[]; vertical: Relation[]; weights: Map<string, number> } {
  ...
  for (const edge of edges) {
    ...
    if (Math.abs(dx) >= Math.abs(dy)) {
      ...
      if (dx >= 0) {
        relation = [edge.pointA.id, edge.pointB.id];
      }
      else {
        relation = [edge.pointB.id, edge.pointA.id];
      }
      horizontal.push(relation);
    }
    else {
      ...
      vertical.push(relation);
    }
    // Synthetic low weight: a bridge is a weak, long-distance stitch between
    // otherwise disconnected components, so it must rank below every real bond.
    weights.set(`${relation[0]}\0${relation[1]}`, BRIDGE_WEIGHT);
  }

  return { horizontal, vertical, weights };
}
