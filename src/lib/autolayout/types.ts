...
/**
 * Edge weights for a {@link LayoutInput} constraint graph.
 *
 * Maps a directed edge to a scalar affinity in `(0, 1]`, keyed by the same
 * `from\u0000to` encoding used elsewhere for edges. Produced by
 * {@link findNeighbors} (facing-edge overlap) and {@link bridgeSets} (a low
 * synthetic weight for stitched components). {@link layout} uses these weights
 * only to *guide placement order* — a higher weight means "place this pair
 * adjacently first" — never to relax a constraint.
 */
export type EdgeWeights = ReadonlyMap<string, number>;
...
export interface LayoutInput {
  ...
  /**
   * Optional center coordinates for each node (in the same units as the input
   * rectangles). When provided, {@link layout} uses them to reconstruct
   * physical columns (grouping vertically-connected keys and ordering columns
   * left-to-right by x) — essential for staggered / split keyboards where the
   * horizontal graph alone cannot order the two halves. Omit to fall back to a
   * generic constructive placement.
   */
  coords?: ReadonlyMap<ObjId, { x: number; y: number; width: number; height: number }>;
  /**
   * Optional per-edge affinity weights, keyed by `from\u0000to`. Used by
   * {@link layout} to order placement so that strongly-bonded neighbours are
   * placed adjacently first. Omit to use uniform weights (plain BFS order).
   *
   * @see {@link EdgeWeights}
   */
  weights?: EdgeWeights;
  /**
