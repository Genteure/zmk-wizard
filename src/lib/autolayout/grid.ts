...
function assignSplitIndices(
  ...
  for (const n of entities) {
    ...
    if (kIn >= kOut) {
      ...
      // Greedily merge out-edges into the nearest free dominant slot
      const used = new Set<number>();
      for (const [, target] of sortedOut) {
        const tCoord = approx.get(target) ?? 0;
        let bestIdx = -1;
        let bestDist = Infinity;
        for (let i = 0; i < numClones; i++) {
          if (used.has(i)) continue;
          const refCoord = approx.get(sortedIn[i][0]) ?? 0;
          const d = Math.abs(refCoord - tCoord);
          if (d < bestDist) {
            bestDist = d;
            bestIdx = i;
          }
        }
        edgeSrc.set(edgeKey(n, target), bestIdx);
        used.add(bestIdx);
      }
    } else {
