  ...
  <div class="overflow-y-auto p-2">
    ...
    <UCard class="mt-4" :title="$t('dongle-title')" :description="$t('dongle-description')">
      ...
      <p class="text-toned mt-2">
        lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptas, voluptate. Adipisci, quisquam. Voluptas,
        voluptate.
      </p>
    </UCard>

    <UCard class="mt-4" title="Keymap" description="Shield Wizard does not support editing keymaps.">
      <p class="text-toned mb-2">
        Shield Wizard will generate a default keymap (<code
          class="font-mono text-xs px-1 py-0.5 rounded border">&kp A &kp B &kp C …</code>, looping after Z) for you
        to test the keyboard.
      </p>
      <p class="text-toned mb-2">
        Keymap bindings follow the same order as the keys, which is determined by the keymap layout. Make sure the
        keymap
        layout matches how you think of the keys. The keymap layout does not have to match how the keys are electrically
        wired.
      </p>
      <p class="text-toned">
        Please create your own keymap <em>after</em> creating the repository, either by editing the <code
          class="font-mono text-xs px-1 py-0.5 rounded border">name.keymap</code> file or by using third-party tools
        like Keymap Editor.
      </p>
    </UCard>
  </div>
