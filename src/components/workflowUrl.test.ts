...
describe('workflow URL parsing', () => {
  ...
  it('strips workflow keys but preserves hash and unrelated query', () => {
    const url = new URL('https://example.test/?action=edit&code=abc&state=def&setup_action=install&installation_id=1&repo=a%2Fb&tab=layout&part=0&keep=1#kle=xyz');
    const stripped = stripWorkflowSearch(url);

    expect(stripped).toContain('keep=1');
    expect(stripped).toContain('#kle=xyz');
    for (const key of ['action', 'code', 'state', 'setup_action', 'installation_id', 'repo', 'tab', 'part']) {
